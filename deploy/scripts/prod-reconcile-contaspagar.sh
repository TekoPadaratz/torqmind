#!/usr/bin/env bash
# Reconcile CONTASPAGAR (Xpert -> STG -> CH titles/overview) on the App VM.
#
# Heals "title paid directly in CONTASPAGAR still shown as open" — DTAPGTO/VLRPAGO
# change without DATAREPL bump; agent watermark misses the payment; phantoms stay
# open forever once outside the paid revisit window. Mirrors
# prod-reconcile-contasreceber.sh with close_phantoms.
#
# Usage:
#   ENV_FILE=/etc/torqmind/prod.app.env \
#   SQLSERVER_ENV_FILE=config/source-explorer.env \
#   ID_EMPRESA=1 ./deploy/scripts/prod-reconcile-contaspagar.sh
#
# Suggested cron (App VM, twice daily + lock):
#   10 6,18 * * * cd /home/tm/torqmind && flock -n /tmp/torqmind-reconcile-cp.lock \
#     env ENV_FILE=/etc/torqmind/prod.app.env SQLSERVER_ENV_FILE=config/source-explorer.env \
#     ID_EMPRESA=1 ./deploy/scripts/prod-reconcile-contaspagar.sh \
#     >> /home/tm/logs/reconcile-contaspagar-cron.log 2>&1
set -euo pipefail
...
ARGS=(scripts/fix_contaspagar_sync.py
...
echo "[reconcile-contaspagar] $(date -Is) id_empresa=$ID_EMPRESA paid_days=$PAID_DAYS dry_run=$DRY_RUN"
"$PYBIN" "${ARGS[@]}"

if [[ "$DRY_RUN" == "1" || "$SKIP_PUBLISH" == "1" ]]; then
  echo "[reconcile-contaspagar] skip publish (dry_run=$DRY_RUN skip_publish=$SKIP_PUBLISH)"
  exit 0
fi

echo "[reconcile-contaspagar] publish mart_finance_titles_rt"
ENV_FILE="$ENV_FILE" ID_EMPRESA="$ID_EMPRESA" DAYS="$PAID_DAYS" \
  ./deploy/scripts/publish-finance-titles.sh

echo "[reconcile-contaspagar] wait CDC STG→CH then refresh finance_overview_rt"
# Debezium usually catches upserts in seconds; brief wait before overview rebuild.
sleep 12
...
echo "[reconcile-contaspagar] DONE $(date -Is)"
