...
echo "--- Raw Events ---"
RAW_COUNT=$(docker exec "$CH_CONTAINER" clickhouse-client --query "SELECT count() FROM torqmind_raw.cdc_events WHERE id_empresa = $ID_EMPRESA" 2>/dev/null || echo "0")
...
for table in "${TABLES[@]}"; do
    count=$(docker exec "$CH_CONTAINER" clickhouse-client --query "SELECT count() FROM torqmind_current.$table FINAL WHERE id_empresa = $ID_EMPRESA AND is_deleted = 0" 2>/dev/null || echo "-1")
    if [[ "$count" == "-1" ]]; then
...
if [[ -n "$PG_CONTAINER" ]]; then
    PG_COUNT=$(docker exec "$PG_CONTAINER" psql -U "${PG_USER:-postgres}" -d "${PG_DATABASE:-torqmind}" -tAc "SELECT count(*) FROM dw.fact_venda WHERE id_empresa = $ID_EMPRESA" 2>/dev/null || echo "?")
    CH_COUNT=$(docker exec "$CH_CONTAINER" clickhouse-client --query "SELECT count() FROM torqmind_current.fact_venda FINAL WHERE id_empresa = $ID_EMPRESA AND is_deleted = 0" 2>/dev/null || echo "?")
...
echo ""
echo "--- Errors ---"
ERROR_COUNT=$(docker exec "$CH_CONTAINER" clickhouse-client --query "SELECT count() FROM torqmind_ops.cdc_errors" 2>/dev/null || echo "0")
