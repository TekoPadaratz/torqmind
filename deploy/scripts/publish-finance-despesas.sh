#!/usr/bin/env bash
# Publica despesas (CAP×plano) e funcionários ativos → mart ClickHouse.
set -euo pipefail
...
cd "$ROOT"
.venv/bin/python - <<PY
from app.services.finance_despesas import publish_finance_despesas, publish_team_employees
import json
d = publish_finance_despesas("${ROLE}", int("${EMPRESA}"), days=int("${DAYS}"))
e = publish_team_employees("${ROLE}", int("${EMPRESA}"))
print(json.dumps({"ok": True, "despesas": d, "employees": e, "id_empresa": int("${EMPRESA}")}))
PY
