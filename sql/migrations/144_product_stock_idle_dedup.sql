-- 144: Dedup estoque/produto no refresh Gestão de Produtos (múltiplos id_db em stg.estoque)
BEGIN;
...
BEGIN
  ...
  prod AS (
    ...
    ORDER BY p.id_empresa, p.id_filial, p.id_produto, p.updated_at DESC NULLS LAST
  ),
  purchase_lines AS (
    SELECT
      i.id_empresa,
