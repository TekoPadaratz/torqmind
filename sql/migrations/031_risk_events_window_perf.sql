...
CREATE INDEX IF NOT EXISTS ix_fact_comprovante_risk_cancel_window
  ON dw.fact_comprovante (id_empresa, data_key, id_filial, id_usuario, data)
  INCLUDE (id_db, id_comprovante, id_turno, id_cliente, valor_total)
  WHERE cancelado = true AND data IS NOT NULL;

CREATE INDEX IF NOT EXISTS ix_fact_venda_risk_user_window
  ON dw.fact_venda (id_empresa, id_filial, id_usuario, data_key, data)
  INCLUDE (id_db, id_comprovante, id_movprodutos, id_turno, id_cliente, cancelado)
  WHERE data IS NOT NULL AND id_usuario IS NOT NULL;

CREATE INDEX IF NOT EXISTS ix_fact_venda_item_risk_cover
...
BEGIN
  ...
  IF v_max_source_ts IS NULL THEN
    PERFORM etl.set_watermark(p_id_empresa, 'risk_events', COALESCE(p_end_ts, now()), NULL::bigint);
    RETURN 0;
  END IF;

  v_anchor_ts := COALESCE(p_end_ts, v_max_source_ts, now());
  v_effective_end_ts := COALESCE(p_end_ts, v_max_source_ts, now());

  IF p_force_full THEN
    v_start_ts := COALESCE(v_min_source_ts, v_anchor_ts - interval '90 days');
  ELSE
    v_start_ts := COALESCE(v_wm, v_anchor_ts - make_interval(days => p_lookback_days)) - interval '1 day';
  END IF;

  IF v_effective_end_ts < v_start_ts THEN
    v_effective_end_ts := v_start_ts;
  END IF;

  v_window_end_ts := v_effective_end_ts + interval '1 day';
  v_end_key := etl.date_key((v_effective_end_ts AT TIME ZONE 'UTC')::timestamp);
  ...
  DROP TABLE IF EXISTS tmp_risk_comprovantes_window;
  CREATE TEMP TABLE tmp_risk_comprovantes_window ON COMMIT DROP AS
  SELECT
    c.id_empresa,
    c.id_filial,
    c.data_key,
  ...
  WHERE c.id_empresa = p_id_empresa
    ...
    AND c.data < v_window_end_ts;

  CREATE INDEX tmp_risk_comprovantes_window_main_idx
    ON tmp_risk_comprovantes_window (id_filial, data_key, data);
  CREATE INDEX tmp_risk_comprovantes_window_user_idx
    ON tmp_risk_comprovantes_window (id_filial, id_usuario, data);
  CREATE INDEX tmp_risk_comprovantes_window_doc_idx
    ON tmp_risk_comprovantes_window (id_filial, id_db, id_comprovante);

  DROP TABLE IF EXISTS tmp_risk_sales_window;
  CREATE TEMP TABLE tmp_risk_sales_window ON COMMIT DROP AS
  SELECT
    v.id_empresa,
    v.id_filial,
    v.data_key,
  ...
  FROM dw.fact_venda v
  WHERE v.id_empresa = p_id_empresa
    AND v.data_key BETWEEN v_start_key_30 AND v_end_key
    AND v.data IS NOT NULL
    AND v.data >= (v_start_ts - interval '30 days')
    AND v.data < v_window_end_ts;

  CREATE INDEX tmp_risk_sales_window_main_idx
    ON tmp_risk_sales_window (id_filial, data_key, data);
  CREATE INDEX tmp_risk_sales_window_user_idx
    ON tmp_risk_sales_window (id_filial, id_usuario, data);
  CREATE INDEX tmp_risk_sales_window_doc_idx
    ON tmp_risk_sales_window (id_filial, id_db, id_comprovante);
  CREATE INDEX tmp_risk_sales_window_mov_idx
    ON tmp_risk_sales_window (id_filial, id_db, id_movprodutos);

  DROP TABLE IF EXISTS tmp_risk_sale_items_window;
  CREATE TEMP TABLE tmp_risk_sale_items_window ON COMMIT DROP AS
  SELECT
    s.id_empresa,
    s.id_filial,
    s.data_key,
  ...
  CREATE INDEX tmp_risk_sale_items_window_mov_idx
    ON tmp_risk_sale_items_window (id_filial, id_db, id_movprodutos);
  CREATE INDEX tmp_risk_sale_items_window_data_idx
    ON tmp_risk_sale_items_window (id_filial, data);

  DELETE FROM dw.fact_risco_evento r
  WHERE r.id_empresa = p_id_empresa
    AND (
      r.data_key BETWEEN v_start_key AND v_end_key
      OR (
        r.event_type IN ('CANCELAMENTO', 'CANCELAMENTO_SEGUIDO_VENDA', 'HORARIO_RISCO', 'FUNCIONARIO_OUTLIER')
        AND EXISTS (
          SELECT 1
          FROM tmp_risk_comprovantes_window c
          WHERE c.data_key BETWEEN v_start_key AND v_end_key
            AND c.data >= v_start_ts
            AND c.data < v_window_end_ts
            AND c.id_filial = r.id_filial
            AND COALESCE(c.id_db, -1) = COALESCE(r.id_db, -1)
            AND COALESCE(c.id_comprovante, -1) = COALESCE(r.id_comprovante, -1)
        )
      )
      OR (
        r.event_type = 'DESCONTO_ALTO'
        AND EXISTS (
          SELECT 1
          FROM tmp_risk_sales_window s
          WHERE s.data_key BETWEEN v_start_key AND v_end_key
            AND s.data >= v_start_ts
            AND s.data < v_window_end_ts
            AND s.id_filial = r.id_filial
            AND COALESCE(s.id_db, -1) = COALESCE(r.id_db, -1)
            AND COALESCE(s.id_movprodutos, -1) = COALESCE(r.id_movprodutos, -1)
        )
      )
    );
  ...
  user_stats AS (
    ...
    FROM tmp_risk_comprovantes_window c
    WHERE c.data_key BETWEEN v_start_key_14 AND v_end_key
  ...
  hour_stats AS (
    SELECT
      ...
      COALESCE(COUNT(*) FILTER (WHERE c.cancelado)::numeric / NULLIF(COUNT(*)::numeric, 0), 0) AS cancel_rate
    FROM tmp_risk_comprovantes_window c
    WHERE c.data_key BETWEEN v_start_key_30 AND v_end_key
      AND c.data >= (v_start_ts - interval '30 days')
      AND c.data < v_window_end_ts
    GROUP BY 1,2,3
  ),
  ...
  movement_employee_map AS (
    ...
    FROM tmp_risk_sale_items_window i
  ...
  cancel_base AS (
    SELECT
      ...
      EXISTS (
        SELECT 1
        FROM tmp_risk_sales_window v2
        WHERE v2.id_filial = c.id_filial
    ...
    FROM tmp_risk_comprovantes_window c
    LEFT JOIN tmp_risk_sales_window v
      ON v.id_filial = c.id_filial
     AND v.id_db = c.id_db
  ...
  discount_raw AS (
    ...
    FROM tmp_risk_sale_items_window i
    WHERE i.data_key BETWEEN v_start_key AND v_end_key
      AND i.data >= v_start_ts
      AND i.data < v_window_end_ts
      AND COALESCE(i.cancelado,false) = false
  ...
  price_reference_30d AS (
    ...
    JOIN tmp_risk_sale_items_window i2
      ON i2.id_empresa = dd.id_empresa
     AND i2.id_filial = dd.id_filial
     AND i2.data >= (dd.dt_ref - interval '30 days')
     AND i2.data < dd.dt_ref
    WHERE i2.valor_unitario IS NOT NULL
      AND COALESCE(i2.cancelado, false) = false
    GROUP BY 1,2,3
  ),
  discount_scored AS (
    SELECT
  ...
  PERFORM etl.set_watermark(
    p_id_empresa,
    'risk_events',
    COALESCE(LEAST(v_max_source_ts, v_effective_end_ts), v_effective_end_ts, now()),
    NULL::bigint
  );
