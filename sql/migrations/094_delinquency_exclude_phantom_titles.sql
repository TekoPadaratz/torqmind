...
-- ============================================================
-- 094 — Delinquency mart: exclude reconciled-absent (phantom) titles
-- ============================================================
-- Supersedes the function from 093 (keeps the per-empresa advisory lock) and
-- adds a guard for PHANTOM receivables.
--
-- ROOT CAUSE (2026-06-06): ID_CONTASRECEBER is unique per ID_DB. When a title is
-- DELETED or RENUMBERED in Xpert, the agent/reconcile (UPSERT-only) never closes
-- the old row, so it lingers OPEN in stg.contasreceber -> dw.fact_financeiro
-- (data_pagamento NULL) -> shows as overdue in Prioridades de Cobranca even
-- though it no longer exists in the source. (Titles paid > reconcile window are
-- handled by the reconcile re-fetch; truly-deleted ones get a tombstone.)
--
-- The reconcile (scripts/fix_contasreceber_sync.py close_phantoms) marks such
-- titles in stg.contasreceber payload with key TORQMIND_RECONCILED_ABSENT. That
-- marker flows into dw.fact_financeiro.payload (load_fact_financeiro copies
-- payload on conflict). Here the open-receivables CTE excludes marked titles, so
-- a title that vanished from the source stops counting as an open receivable.
-- The marker is reversible: if the title reappears OPEN in Xpert, the reconcile
-- upsert overwrites payload (marker gone) and it reopens.
--
-- Idempotent: CREATE OR REPLACE FUNCTION. Applied in isolation (prod migration
-- chain 076-087 is known-broken; do NOT run the migrate CLI here).
-- ============================================================
...
DECLARE
  ...
  v_ref_date date;
BEGIN
  -- Serialize concurrent refreshes per empresa (orchestrator vs reconcile cron
  -- vs manual). Transaction-scoped: auto-released at COMMIT/ROLLBACK. Prevents
  -- the DELETE+INSERT race that aborted the refresh and left the mart stale.
  PERFORM pg_advisory_xact_lock(
    hashtext('etl.refresh_customer_delinquency_summary'),
    COALESCE(p_id_empresa, 0)
  );
  ...
  WITH open_financeiro AS (
    -- All open receivables (unpaid). Exclude phantom titles that were deleted/
    -- renumbered in the source and tombstoned by the reconcile (no longer a
    -- real open receivable).
    SELECT
      f.id_empresa,
      f.id_filial,
      f.id_titulo,
    ...
    WHERE (p_id_empresa IS NULL OR f.id_empresa = p_id_empresa)
      ...
      AND NOT (f.payload ? 'TORQMIND_RECONCILED_ABSENT')
  ), baixa_agg AS (
