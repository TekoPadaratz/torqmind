-- 120_fraud_credito_funcionario_limites_entidade.sql
-- Limites canônicos no cadastro de ENTIDADES (join por CPF), não FUNCIONARIOS.LIMITEVALE.
--   ENTIDADES.LIMITE       → limite a prazo
--   ENTIDADES.LIMITE_VALE  → limite de vale
--   total = soma dos dois
-- Uso: CONTASRECEBER do cliente-espelho; classifica vale se HISTORICO ~ 'vale', senão a prazo.
-- Documento na API: somente NF-e/NFC-e (não NROCOMPROVANTE).

BEGIN;

ALTER TABLE mart.fraud_credito_funcionario_resumo
  ADD COLUMN IF NOT EXISTS limite_prazo numeric(18,2) NOT NULL DEFAULT 0;

ALTER TABLE mart.fraud_credito_funcionario_resumo
  ADD COLUMN IF NOT EXISTS limite_total numeric(18,2) NOT NULL DEFAULT 0;

ALTER TABLE mart.fraud_credito_funcionario_resumo
  ADD COLUMN IF NOT EXISTS usado_prazo numeric(18,2) NOT NULL DEFAULT 0;

ALTER TABLE mart.fraud_credito_funcionario_resumo
  ADD COLUMN IF NOT EXISTS usado_vale numeric(18,2) NOT NULL DEFAULT 0;

ALTER TABLE mart.fraud_credito_funcionario_resumo
  ADD COLUMN IF NOT EXISTS saldo_prazo numeric(18,2) NOT NULL DEFAULT 0;

ALTER TABLE mart.fraud_credito_funcionario_resumo
  ADD COLUMN IF NOT EXISTS saldo_vale numeric(18,2) NOT NULL DEFAULT 0;

ALTER TABLE mart.fraud_credito_funcionario_uso
  ADD COLUMN IF NOT EXISTS tipo_uso text NOT NULL DEFAULT 'prazo';

COMMENT ON COLUMN mart.fraud_credito_funcionario_resumo.limite_prazo IS
  'ENTIDADES.LIMITE (a prazo) via join CPF funcionário↔entidade.';
COMMENT ON COLUMN mart.fraud_credito_funcionario_resumo.limite_vale IS
  'ENTIDADES.LIMITE_VALE (vale) via join CPF. NÃO é FUNCIONARIOS.LIMITEVALE.';
COMMENT ON COLUMN mart.fraud_credito_funcionario_resumo.limite_total IS
  'limite_prazo + limite_vale.';
COMMENT ON COLUMN mart.fraud_credito_funcionario_uso.tipo_uso IS
  'prazo | vale — classificação pelo HISTORICO do título (vale) ou padrão a prazo.';
...
BEGIN
  ...
  SELECT
    ...
    f.vales_cadastro,
    e.id_entidade,
    COALESCE(e.limite_prazo, 0) AS limite_prazo,
    COALESCE(e.limite_vale, 0) AS limite_vale,
    (COALESCE(e.limite_prazo, 0) + COALESCE(e.limite_vale, 0)) AS limite_total
  FROM funcionarios f
  INNER JOIN entidades e
    ON e.id_empresa = f.id_empresa AND e.cpf = f.cpf
  WHERE COALESCE(e.limite_prazo, 0) > 0
     OR COALESCE(e.limite_vale, 0) > 0;
  ...
  WHERE b.id_entidade IS NOT NULL
    AND (
      ...
      OR cr.payload->>'HISTORICO' ILIKE '%NFC-e%'
      OR cr.payload->>'HISTORICO' ILIKE '%NF-e%'
    )
    AND COALESCE(
      NULLIF(cr.payload->>'DTACONTA', '')::timestamptz,
      NULLIF(cr.payload->>'DATAREPL', '')::timestamptz,
  ...
  SELECT
    ...
    r.tipo_uso,
    r.nro_cupom,
    -- Documento canônico = NF; cupom só serve para join técnico do comprovante.
...
COMMENT ON FUNCTION etl.refresh_fraud_credito_funcionario(integer, integer) IS
  'Refresh crédito funcionário: limites ENTIDADES.LIMITE + LIMITE_VALE; uso CONTASRECEBER; doc = NF.';
