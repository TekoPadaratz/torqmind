-- 139: crédito funcionário — OBS (Observações) do CONTASRECEBER Xpert
-- Mantém HISTORICO para vínculo fiscal (NFC-e/Cupom); OBS é o texto livre operacional.
-- Exibição: coalesce(nullif(trim(OBS),''), HISTORICO).
...
COMMENT ON COLUMN mart.fraud_credito_funcionario_uso.observacao IS
  'CONTASRECEBER.payload.OBS (Observações no Xpert); preferir na UI quando preenchido.';
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(NULLIF(TRIM(cr.payload->>'OBS'), ''), '') AS observacao,
    CASE
  ...
  INSERT INTO mart.fraud_credito_funcionario_uso (
    ...
    dt_evento, valor, id_usuario_caixa, operador_caixa, historico, observacao, atipico, tipo_uso, refreshed_at
  )
  SELECT
    id_empresa, id_funcionario, v_ano_mes, id_filial, id_entidade,
    id_contasreceber, id_comprovante, nro_cupom, nro_documento,
    dt_evento, valor, id_usuario_caixa, COALESCE(operador_caixa, ''), historico,
    COALESCE(observacao, ''), atipico,
...
COMMENT ON FUNCTION etl.refresh_fraud_credito_funcionario(integer, integer) IS
  'Refresh crédito funcionário: ENTIDADES grupo 12; CR HISTORICO+OBS; dt_evento; uso empresa-wide; doc=NF.';
