-- 149: Gestão de Produtos — campos Xpert (NOMEPRODUTO, CUSTOMEDIO) + preço última venda DW
BEGIN;
...
BEGIN
  ...
  last_sale_price AS (
    ...
    WHERE fvi.id_empresa = p_id_empresa
      AND fvi.data_key >= 20190101
      AND fvi.data_key < 20310101
      AND COALESCE(fvi.preco_praticado_unitario, fvi.valor_unitario, 0) > 0
  ...
  merged AS (
    SELECT
      ...
      COALESCE(NULLIF(lsp.preco_venda, 0), NULLIF(pr.preco_stg, 0), 0) AS preco_venda
    FROM stock_pos s
    LEFT JOIN prod pr
    ...
    LEFT JOIN last_sale_price lsp
      ON lsp.id_empresa = s.id_empresa
     AND lsp.id_filial = s.id_filial
     AND lsp.id_produto = s.id_produto
...
COMMENT ON FUNCTION etl.refresh_product_stock_idle(integer) IS
  'Estoque parado: nomes via NOMEPRODUTO/dim_produto; custo CUSTOMEDIO/compras; preço última venda DW.';

-- Backfill custo_medio em dim_produto (watermark incremental não relê histórico)
UPDATE dw.dim_produto d
SET custo_medio = s.custo_medio
FROM (
  SELECT DISTINCT ON (p.id_empresa, p.id_filial, p.id_produto)
    p.id_empresa,
    p.id_filial,
...
WHERE d.id_empresa = s.id_empresa
  AND d.id_filial = s.id_filial
  AND d.id_produto = s.id_produto
  AND s.custo_medio > 0
  AND (d.custo_medio IS NULL OR d.custo_medio = 0);
