-- 151: Compras recentes — itenscomprovantes (NF entrada) + movprodutos (loja; paridade Xpert)
BEGIN;
...
BEGIN
  ...
  purchase_lines_ic AS (
    SELECT
      ...
      COALESCE(
        NULLIF(trim(both FROM n.chave_acesso_shadow), ''),
...
COMMENT ON FUNCTION etl.refresh_product_stock_idle(integer) IS
  'Estoque parado: compras via itenscomprovantes+nfe_entrada e movprodutos (loja); timeout 20min.';
