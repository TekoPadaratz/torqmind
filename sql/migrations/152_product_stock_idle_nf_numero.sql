-- 152: Documento = número da NF (não chave) + custo unitário honesto nas compras recentes
BEGIN;
...
BEGIN
  ...
  purchase_lines_ic AS (
    SELECT
      ic.id_empresa,
      ic.id_filial,
      COALESCE(ic.id_produto_shadow, etl.safe_int(ic.payload->>'ID_PRODUTOS')) AS id_produto,
      COALESCE(
        ...
        CASE
          WHEN length(trim(both FROM coalesce(n.chave_acesso_shadow, ''))) = 44
            THEN ltrim(substring(trim(both FROM n.chave_acesso_shadow) from 26 for 9), '0')
          ELSE NULL
        END,
        '—'
      ) AS numero_documento,
      ...
      GREATEST(
        COALESCE(
          ic.qtd_shadow,
          etl.safe_numeric(ic.payload->>'QTDE'),
          etl.safe_numeric(ic.payload->>'QUANTIDADE'),
          0
        ),
        0
      )::numeric(18, 3) AS qtd,
      GREATEST(
        COALESCE(
          etl.safe_numeric(ic.payload->>'VLRCUSTO'),
          etl.safe_numeric(ic.payload->>'VLRUNITARIO'),
          etl.safe_numeric(ic.payload->>'CUSTO_UNITARIO'),
          0
        ),
        0
      ...
      GREATEST(
        COALESCE(
          etl.safe_numeric(ic.payload->>'VLRTOTALITEM'),
          etl.safe_numeric(ic.payload->>'VALORTOTAL'),
          0
        ),
        0
    ...
    FROM stg.itenscomprovantes ic
    INNER JOIN stg.nfe_entrada n
      ON n.id_empresa = ic.id_empresa
     AND n.id_filial = ic.id_filial
     AND n.id_db = ic.id_db
     AND n.id_nota = ic.id_comprovante
    INNER JOIN stock_pos sp
      ON sp.id_empresa = ic.id_empresa
     AND sp.id_filial = ic.id_filial
     AND sp.id_produto = COALESCE(ic.id_produto_shadow, etl.safe_int(ic.payload->>'ID_PRODUTOS'))
    WHERE ic.id_empresa = p_id_empresa
      AND COALESCE(ic.id_produto_shadow, etl.safe_int(ic.payload->>'ID_PRODUTOS')) > 0
  ),
  ...
  purchase_lines AS (
    SELECT * FROM purchase_lines_ic
    UNION ALL
    SELECT mov.*
    FROM purchase_lines_mov mov
    WHERE NOT EXISTS (
      SELECT 1
      FROM purchase_lines_ic ic
      WHERE ic.id_empresa = mov.id_empresa
        AND ic.id_filial = mov.id_filial
        AND ic.id_produto = mov.id_produto
        AND ic.data_compra = mov.data_compra
        AND ic.qtd = mov.qtd
        AND ic.numero_documento IS NOT DISTINCT FROM mov.numero_documento
    )
  ),
  purchase_ranked AS (
    SELECT
  ...
  ins_purchase AS (
    ...
    SELECT
      ...
      qtd,
      CASE
        WHEN valor_unitario > 0 THEN valor_unitario
        WHEN valor_total > 0 AND qtd > 0 THEN ROUND(valor_total / qtd, 4)
        ELSE 0
      END,
      CASE
...
COMMENT ON FUNCTION etl.refresh_product_stock_idle(integer) IS
  'Estoque parado: documento=número NF; custo unitário nas compras; timeout 20min.';
