-- 122: Hotfix prod — remove filtro amplo HISTORICO ILIKE %NFC-e%/%NF-e%
-- (explodia o mash em CONTASRECEBER). Mantém nome do funcionário / Cliente:id_entidade.
-- 121 base: operador via NFE→comprovante; documento = NF.
-- Documento na tela continua = NF-e/NFC-e.
-- Caminhos (nessa ordem):
--   1) Cupom no HISTORICO → stg.comprovantes (NROCUPOMFISCAL/NROCOMPROVANTE)
--   2) Número NF-e/NFC-e → stg.nfe (numero_nfe_shadow/NRONF) → id_comprovante → comprovantes
-- Sem comprovante resolvido, operador fica vazio (API/publish enriquecem via CH se preciso).
...
COMMENT ON FUNCTION etl.refresh_fraud_credito_funcionario(integer, integer) IS
  'Refresh crédito funcionário: limites ENTIDADES; CR por nome/Cliente; operador via cupom ou NFE→comprovante; doc=NF.';
