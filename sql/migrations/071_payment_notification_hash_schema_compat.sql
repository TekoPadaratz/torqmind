-- ============================================================================
-- Migration 071: Payment notification hash schema compatibility
-- ============================================================================
...
BEGIN
  WITH src AS (
    SELECT
      ...
      COALESCE(
        CASE
          WHEN p.insight_id_hash IS NULL THEN NULL
          WHEN p.insight_id_hash::text ~ '^-?[0-9]+$' THEN p.insight_id_hash::text::bigint
          ELSE etl.bigint_hash64(p.insight_id_hash::text)
        END,
        CASE WHEN p.insight_id IS NOT NULL THEN etl.bigint_hash64(p.insight_id::text) END
      ) AS insight_id,
