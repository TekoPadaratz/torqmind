...
-- ============================================================
-- 093 — Serialize customer_delinquency_summary refresh (anti-race)
-- ============================================================
-- ROOT CAUSE (2026-06-06): refresh_customer_delinquency_summary does a
-- non-atomic DELETE + INSERT for the whole empresa. Two schedulers call it:
--   1. the */2 ETL orchestrator (post_refresh, on finance_changed), and
--   2. the twice-daily CONTASRECEBER reconcile cron.
-- When they overlap, both DELETE the empresa rows then INSERT the same
-- (id_empresa, id_filial, id_cliente) keys -> the second commit fails with
--   duplicate key value violates unique constraint "customer_delinquency_summary_pkey"
-- The failing transaction rolls back, so STG/DW get updated but the mart is
-- left STALE until the next successful refresh. That is why receivables kept
-- diverging from Xpert (paid titles still shown as overdue overnight).
--
-- FIX: take a per-empresa transaction-level advisory lock at the very start of
-- the function. Concurrent callers now serialize (wait) instead of racing, so
-- the DELETE+INSERT is always consistent and the mart never stays stale due to
-- a refresh crash. Auto-released on COMMIT/ROLLBACK. No data-semantics change.
--
-- Idempotent: CREATE OR REPLACE FUNCTION. Body identical to migration 084 plus
-- the advisory lock. Applied in isolation (the prod migration chain 076-087 is
-- known-broken; do NOT run the migrate CLI here).
-- ============================================================
