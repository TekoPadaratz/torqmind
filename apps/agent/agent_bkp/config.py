...
DEFAULT_DATASETS: Dict[str, Dict[str, Any]] = {
    ...
    "comprovantes": {
        ...
        "watermark_column": "DATAREPL",
        "revisit_open_clause": "CAST(DATA AS date) >= CAST(DATEADD(day,-7,GETDATE()) AS date)",
        "enabled": True,
    },
    "movprodutos": {
    ...
    "formas_pgto_comprovantes": {
        ...
        "query": (
            "SELECT fp.* "
            "FROM dbo.FORMAS_PGTO_COMPROVANTES fp "
            "LEFT JOIN dbo.COMPROVANTES c "
            "  ON c.ID_FILIAL = fp.ID_FILIAL "
            " AND c.ID_DB = fp.ID_DB "
            " AND c.REFERENCIA = fp.ID_REFERENCIA"
        ),
        "watermark_column": "DATAREPL",
        "watermark_expression": "fp.DATAREPL",
        "revisit_open_clause": "CAST(c.DATA AS date) >= CAST(DATEADD(day,-7,GETDATE()) AS date)",
        "enabled": True,
    },
