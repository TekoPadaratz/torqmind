...
def _show_safe_config(raw: Dict[str, Any], logger) -> None:
    lines = [
        ...
        ("api.route_prefix", _get_nested(raw, "api.route_prefix")),
        ("api.ingest_key", _get_nested(raw, "api.ingest_key")),
        ("runtime.interval_seconds", _get_nested(raw, "runtime.interval_seconds")),
        ("runtime.connect_timeout_seconds", _get_nested(raw, "runtime.connect_timeout_seconds")),
        ("runtime.read_timeout_seconds", _get_nested(raw, "runtime.read_timeout_seconds")),
        ("runtime.summary_log_file", _get_nested(raw, "runtime.summary_log_file")),
        ("runtime.log_level", _get_nested(raw, "runtime.log_level")),
    ]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run.add_argument("--dataset", default=None, help="Process only one dataset in the cycle")
    ...
    ping = sub.add_parser("ping", help="Resolve the published API root and test reachability")
    ping.add_argument("--config", dest="command_config", default=None, help="Path to config file")

    test_api = sub.add_parser("test-api", help="Test reachability, ingest auth and empty POST to dataset")
    test_api.add_argument("--config", dest="command_config", default=None, help="Path to config file")
    test_api.add_argument("--dataset", default="filiais", help="Dataset used for empty POST validation")

    send_file = sub.add_parser("send-file", help="Send one NDJSON or NDJSON.GZ file to the ingest endpoint")
    send_file.add_argument("--config", dest="command_config", default=None, help="Path to config file")
    send_file.add_argument("--dataset", required=True)
    send_file.add_argument("--file", required=True, help="Path to .ndjson or .ndjson.gz file")

    spool_status = sub.add_parser("spool-status", help="Show pending/dead-letter spool counters")
    spool_status.add_argument("--config", dest="command_config", default=None, help="Path to config file")

    turnos_status = sub.add_parser("turnos-status", help="Show persistent revisit state for pending TURNOS")
    turnos_status.add_argument("--config", dest="command_config", default=None, help="Path to config file")
...
def main() -> None:
    ...
    api_only_commands = {"ping", "test-api", "send-file", "spool-status"}
    cfg = load_config(
        config_path,
    ...
    if args.command in api_only_commands:
        ...
        try:
            if args.command == "ping":
                result = sink.ping()
                logger.info(
                    "command=ping status=ok public_base_url=%s resolved_api_root=%s public_health_status=%s ingest_health_status=%s",
            ...
            if args.command == "test-api":
                ping_result = sink.ping()
                sink.validate_ingest_credentials()
                post_result = sink.test_ingest_endpoint(dataset=args.dataset)
                logger.info(
                    "command=test-api status=ok resolved_api_root=%s dataset=%s post_status=%s",
                    ping_result["resolved_api_root"],
                    args.dataset,
            ...
            if args.command == "send-file":
                result = sink.send_file(dataset=args.dataset, file_path=args.file)
                logger.info(
                    "command=send-file status=ok dataset=%s inserted_or_updated=%s rejected=%s spooled=%s",
                    args.dataset,
                    result.get("inserted_or_updated"),
                    result.get("rejected"),
                    result.get("spooled", False),
                )
                return

            if args.command == "spool-status":
                stats = sink.spool_status()
                logger.info(
                    "command=spool-status pending_files=%s pending_rows=%s dead_letter_files=%s",
                    stats.get("pending_files", 0),
                    stats.get("pending_rows", 0),
                    stats.get("dead_letter_files", 0),
                )
                return
        except Exception as exc:  # noqa: PERF203
    ...
    try:
        ...
        if args.command == "run":
            ...
            if args.once or not args.loop:
                runner.run_once(
                    only_dataset=(str(args.dataset).strip().lower() if args.dataset else None),
        ...
        if args.command == "turnos-status":
            status = runner.turnos_status()
            logger.info(
                "command=turnos-status status=ok scope=%s pending_total=%s open_pending=%s closed_pending=%s",
                status["scope"],
                status["pending_total"],
                status["open_pending"],
                status["closed_pending"],
            )
            for item in status["items"][:50]:
                logger.info(
                    "command=turnos-status item id_filial=%s id_turnos=%s id_usuarios=%s last_known_status=%s last_delivery=%s missing_count=%s last_seen_at=%s",
                    item.get("id_filial"),
                    item.get("id_turnos"),
                    item.get("id_usuarios"),
                    item.get("last_known_status"),
                    item.get("last_delivery"),
