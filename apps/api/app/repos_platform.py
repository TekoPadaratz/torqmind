...
def _validate_username_or_raise(value: Any) -> str:
    try:
        return validate_username(str(value or ""))
    except ValueError as exc:
        raise AuthError(422, "validation_error", str(exc))


def _exclude_user_clause() -> str:
    return "(%s::uuid IS NULL OR id <> %s::uuid)"


def _user_row_exists(conn, where_sql: str, params: tuple[Any, ...]) -> bool:
    row = conn.execute(f"SELECT 1 FROM auth.users WHERE {where_sql} LIMIT 1", params).fetchone()
    return bool(row)
...
def _assert_user_identity_available(
    ...
    *,
    ...
    exclude_clause = _exclude_user_clause()
    exclude_params = (exclude_user_id, exclude_user_id)

    if _user_row_exists(conn, f"{exclude_clause} AND username = %s", exclude_params + (username,)):
        raise AuthError(409, "username_conflict", "Nome de usuário já está em uso.")

    if _user_row_exists(conn, f"{exclude_clause} AND lower(email) = %s", exclude_params + (email,)):
        raise AuthError(409, "email_conflict", "Email já está em uso.")

    if _user_row_exists(conn, f"{exclude_clause} AND lower(email) = %s", exclude_params + (username,)):
        raise AuthError(409, "username_conflict", "Nome de usuário conflita com o email de outro usuário.")

    if _user_row_exists(conn, f"{exclude_clause} AND username = %s", exclude_params + (email,)):
        raise AuthError(409, "email_conflict", "Email conflita com o nome de usuário de outro usuário.")
        ...
        if user_id:
            ...
            if not _actor_is_sovereign(claims) and is_sovereign_email(payload["email"]):
                if str(payload["email"]).strip().lower() != str(previous.get("email") or "").strip().lower():
                    raise AuthError(403, "sovereign_user_protected", "Usuário soberano protegido.")
            _assert_user_identity_available(conn, email=payload["email"], username=payload["username"], exclude_user_id=user_id)
            password_sql = ""
            params: list[Any] = [
                payload["nome"],
                payload["email"],
            ...
            _assert_user_identity_available(conn, email=payload["email"], username=payload["username"])
