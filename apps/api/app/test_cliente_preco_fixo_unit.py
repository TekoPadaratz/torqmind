...
class ClientePrecoFixoUnitTests(unittest.TestCase):
    ...
    def test_detail_maps_document_and_prices(self) -> None:
        ch_rows = [
            {
        ...
        with patch.object(svc, "query_dict", side_effect=[
            [{"n": 1, "desconto_total_sum": 5.56}],
            ch_rows,
        ]), patch.object(svc, "apelido_for", return_value="VR01"):
            out = svc.detail(1, 10169, 7527, date(2026, 7, 1), date(2026, 7, 20))
        self.assertEqual(out["filial_label"], "VR01")
        self.assertEqual(out["items"][0]["documento_label"], "325152")
        self.assertAlmostEqual(out["items"][0]["preco_bomba"], 6.52)
        self.assertAlmostEqual(out["items"][0]["preco_pago"], 6.32)
        self.assertAlmostEqual(out["items"][0]["desconto_total"], 5.56)
