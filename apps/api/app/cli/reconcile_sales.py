    ...
    row = conn.execute(
        ...
        WITH source_items AS (
          ...
          WHERE i.id_empresa = %s
            ...
            AND etl.movimento_venda_is_cancelled(etl.movimento_venda_situacao(m.situacao_shadow, m.payload)) = false
            AND {group_expr} = %s
        )
        SELECT
...
def _fetch_top_extra_items(
    ...
    items = conn.execute(
        ...
        SELECT
          ...
          v.id_movprodutos,
          COALESCE(i.id_itemcomprovante, i.id_itensmovprodutos) AS id_itemcomprovante,
