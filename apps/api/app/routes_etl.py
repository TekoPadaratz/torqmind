...
def run_etl(
    ...
    effective_ref_date = resolve_business_date(ref_date, tenant_id=id_empresa or claims.get("id_empresa"))
    try:
...
def run_micro_risk(
    ...
    with get_conn(role="MASTER", tenant_id=id_emp, branch_id=filial_scope) as conn:
        ...
        freshness_limit_key = int((business_today(id_emp) - timedelta(days=hot_days)).strftime("%Y%m%d"))
        ...
        for r in recent:
            ...
            dt_ref = business_date_for_datetime(last_event_at, id_emp)
