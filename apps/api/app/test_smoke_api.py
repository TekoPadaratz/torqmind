...
class SmokeApiTest(unittest.TestCase):
    ...
    def test_bi_dashboard_overview_returns_data(self) -> None:
        ...
        self.assertIn("primary_shortcut", home["overview"]["jarvis"])
    ...
    def test_customers_overview_returns_customer_names_for_real_branch(self) -> None:
        ...
        self.assertIn("delinquency", body)
        self.assertIn("summary", body["delinquency"])
        self.assertIn("buckets", body["delinquency"])
        self.assertIn("customers", body["delinquency"])

    def test_sales_overview_endpoint_returns_contract(self) -> None:
        status, body = self._request(
            "/bi/sales/overview?dt_ini=2025-09-01&dt_fim=2025-09-18&dt_ref=2025-09-18&id_empresa=1",
            headers={"Authorization": f"Bearer {self.token}"},
        )
        self.assertEqual(status, 200)
        self.assertIn("kpis", body)
        ...
        self.assertIn("top_groups", body)
        self.assertIn("top_employees", body)
        self.assertIn("commercial_kpis", body)
        self.assertIn("commercial_by_hour", body)
        self.assertIn("cfop_breakdown", body)
        self.assertIn("monthly_evolution", body)

    def test_sync_status_endpoint_returns_contract(self) -> None:
        status, body = self._request(
            "/bi/sync/status?id_empresa=1",
            headers={"Authorization": f"Bearer {self.token}"},
        )
        self.assertEqual(status, 200)
    ...
    def test_cash_overview_endpoint_returns_contract(self) -> None:
        ...
        self.assertIn("commercial", body)
        self.assertIn("dre_summary", body)
