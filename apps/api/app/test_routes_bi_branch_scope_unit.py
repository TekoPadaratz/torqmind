...
class BranchScopeRouteUnitTest(unittest.TestCase):
    ...
    def test_dashboard_home_passes_multi_branch_scope_into_bundle(self) -> None:
        with (
            ...
            patch.object(repos_mart, "dashboard_home_bundle", return_value={"ok": True}) as dashboard_home_bundle,
        ):
    ...
    def test_sales_overview_passes_multi_branch_scope_into_bundle(self) -> None:
        with (
            patch.object(routes_bi, "resolve_scope_filters", return_value=(7, [11, 13], [11, 13])),
            patch.object(repos_mart, "sales_overview_bundle", return_value={"ok": True}) as sales_overview_bundle,
        ):
    ...
    def test_cash_overview_passes_multi_branch_scope_into_repo(self) -> None:
        with (
            patch.object(routes_bi, "resolve_scope_filters", return_value=(7, [11, 13], [11, 13])),
            patch.object(repos_mart, "cash_overview", return_value={"ok": True}) as cash_overview,
        ):
    ...
    def test_fraud_overview_passes_multi_branch_scope_into_operational_and_risk_repos(self) -> None:
        with (
            patch.object(routes_bi, "resolve_scope_filters", return_value=(7, [11, 13], [11, 13])),
            patch.object(repos_mart, "fraud_kpis", return_value={"cancelamentos": 0, "valor_cancelado": 0}) as fraud_kpis,
            patch.object(repos_mart, "fraud_series", return_value=[]),
            patch.object(repos_mart, "fraud_top_users", return_value=[]),
            patch.object(repos_mart, "fraud_last_events", return_value=[]),
            patch.object(repos_mart, "risk_kpis", return_value={"total_eventos": 0}),
            patch.object(repos_mart, "risk_series", return_value=[]),
            patch.object(repos_mart, "risk_data_window", return_value={"rows": 0}),
            patch.object(repos_mart, "risk_model_coverage", return_value={"coverage": "ok"}),
            patch.object(repos_mart, "risk_top_employees", return_value=[]),
            patch.object(repos_mart, "risk_by_turn_local", return_value=[]),
            patch.object(repos_mart, "risk_last_events", return_value=[]),
            patch.object(repos_mart, "payments_anomalies", return_value=[]),
            patch.object(repos_mart, "open_cash_monitor", return_value={"source_status": "ok"}) as open_cash_monitor,
