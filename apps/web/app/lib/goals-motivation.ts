...
export function buildGoalsMotivation(rows: RankedSeller[]) {
  if (!rows.length) {
    return {
      headline: 'A classificação ganha força assim que a equipe volta a pontuar.',
      subheadline: 'Sem movimento suficiente no período para montar a leitura competitiva.',
      indicators: [
        { label: 'Na classificação hoje', value: '0', detail: 'aguardando vendas' },
      ],
    };
  }
  ...
  let headline = 'A consistência está decidindo a classificação comercial.';
  ...
  if (second && leadGap <= Math.max(300, leader.faturamento * 0.03)) {
    ...
    headline = 'A disputa pelo pódio está viva e qualquer arrancada muda a conversa.';
    ...
    headline = 'O pelotão de reação segue forte e os 5 primeiros ainda não estão definidos.';
    subheadline = 'Pequenos ganhos agora fazem muita diferença na leitura da reunião.';
  } else if (avgRisk >= 65) {
    headline = 'A classificação avança, mas a disciplina operacional ainda separa as melhores posições.';
    subheadline = 'Quem combinar volume com consistência deve ganhar vantagem nas próximas horas.';
  } else if (top5Sales >= 120) {
    headline = 'O time entrou em ritmo forte e a classificação ficou mais disputa.';
  ...
  return {
    ...
    indicators: [
      ...
      {
        label: 'Disputa pelo pódio',
        value: third && fourth ? formatCurrency(podiumGap) : 'Sem disputa',
        detail: third && fourth ? `${third.funcionario_nome} x ${fourth.funcionario_nome}` : 'pódio consolidado',
      },
      {
        label: 'Na classificação hoje',
        value: String(rows.length),
        detail: 'colaboradores válidos',
      },
      {
        label: 'Vendas dos 5 primeiros',
