...
- **REGRA GLOBAL DOCUMENTO (atualizado 2026-07-20, reforçado)**: **DOCUMENTO = número da NF-e/NFC-e** (`stg_nfe_slim` / `stg.nfe` / parse HISTORICO). Sem NF → `—`. **Proibido** `NROCOMPROVANTE`, `id_comprovante`, `Turno + Filial`, rotular NFC-e como "Cupom", `MOVPRODUTOS`. Label = só o número. Ver `.cursor/rules/07-documento-nota-fiscal.mdc`.
- **Contrato unico** (`repos_mart_realtime._build_antifraude_event` / crédito funcionário / clientes preço fixo): emite `documento_label`, `documento_source` (`nota_fiscal` | `fallback`), `documento_fiscal` quando houver NF.
