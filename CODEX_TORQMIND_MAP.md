...
- **REGRA GLOBAL DOCUMENTO (atualizado 2026-07-20)**: em telas de risco/fraude e drill-downs, o documento exibido é o **número da NF-e/NFC-e** (join `stg_nfe_slim` / parse HISTORICO). Fallback: `NROCOMPROVANTE`. Último recurso: `id_comprovante`. Label = só o número (sem prefixo "Nota"/"Cupom"/"Comprovante"). NUNCA `Turno + Filial`, NUNCA rotular NFC-e como "Cupom", NUNCA `MOVPRODUTOS`.
- **Contrato unico** (`repos_mart_realtime._build_antifraude_event` / crédito funcionário): emite `documento_label`, `documento_source` (`nota_fiscal` | `documento_venda` | `id_comprovante` | `fallback`), `documento_fiscal` quando houver NF.
