...
| 4 | **Alerta caixa aberto >24h no Telegram** | ENTRA. **Correção 2026-06-03:** infra Telegram já é madura (`services/telegram.py` `send_telegram_alert` + `app.telegram_settings`/`telegram_dispatch_log`) e o disparo de caixa **já está codado e ligado** (`_dispatch_cash_telegram_alerts` no `etl_orchestrator`), com mensagem boa (sem "caixa ?"). **Causa-raiz de não chegar: lê `mart.alerta_caixa_aberto` (PG) que está VAZIA pós-cutover realtime; no ClickHouse não há `alerta_caixa_aberto`.** Fix correto = dispatcher realtime lendo `torqmind_mart_rt.cash_overview_rt` (`is_aberto=1`, `>24h`). Não implementado nesta rodada (mexe no hot path realtime, exige validação prod). |
