...
- Grids BI (contrato mestre): colunas **Filial → Data → Documento**; ordenação de linhas **Filial ASC → Data DESC → Nome ASC** (campos ausentes ignorados; sem os três → 1º campo de negócio ASC). Destaques por métrica são exceção. Listagens: 30/página, “Exibindo X–Y de Z”, “Ordem padrão”, totalizadores corretos. Ver `.cursor/rules/08-grids-colunas-ordenacao.mdc`, `useRecordGrid` e `grid-sort.ts`.
