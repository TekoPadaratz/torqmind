...
- Documento operacional da venda (telas de risco/fraude e drill-downs): preferir número da **NF-e/NFC-e** via join em `stg.nfe` / `stg_nfe_slim` (e/ou parse honesto do HISTORICO). Fallback: `NROCOMPROVANTE`. Último recurso: `id_comprovante`. Nunca montar documento como `Turno + Filial`, nunca prefixar "Cupom" em cima de número de nota, nunca voltar a `MOVPRODUTOS`.
