#!/usr/bin/env python3
"""Reconcile CONTASPAGAR / CONTASPAGARBAIXA between Xpert and TorqMind.

Root cause this heals
---------------------
A title PAID directly in ``CONTASPAGAR`` (DTAPGTO/VLRPAGO set on the row) does
NOT bump ``DATAREPL`` in the ERP, and the agent's composite watermark for
``contaspagar`` can be poisoned to a future date by a dirty row. The agent's
...
freezes as "open" in ``stg.contaspagar`` -> ``dw.fact_financeiro`` keeps
``data_pagamento IS NULL`` -> the finance_titles mart shows it as overdue.
...
2. Upserts ``stg.contaspagar`` / ``stg.contaspagarbaixa`` by PK, bumping
   ``received_at`` ONLY when the payment fields actually changed (minimal churn).
3. Syncs ``dw.fact_financeiro`` (tipo_titulo=0) and leaves CH republication to
   ``deploy/scripts/prod-reconcile-contaspagar.sh`` (publish titles + overview).
...
-----
    ...
    python scripts/fix_contaspagar_sync.py \
        --sqlserver-env-file config/source-explorer.env --id-empresa 1
    # preview only:
    python scripts/fix_contaspagar_sync.py ... --dry-run
"""
...
log = logging.getLogger("reconcile_contaspagar")
...
def extract_contaspagar(cfg: dict[str, Any], paid_days: int) -> list[dict[str, Any]]:
    """Open titles + titles paid in the last ``paid_days`` days."""
    sql = (
        "SELECT c.* FROM dbo.CONTASPAGAR c "
    ...
    try:
        ...
        conn.close()
    log.info("Xpert CONTASPAGAR hot-window rows: %d", len(rows))
    return rows


def extract_baixas(cfg: dict[str, Any], paid_days: int) -> list[dict[str, Any]]:
    sql = (
        "SELECT c.* FROM dbo.CONTASPAGARBAIXA c "
    ...
    try:
        ...
        conn.close()
    log.info("Xpert CONTASPAGARBAIXA hot-window rows: %d", len(rows))
    return rows


def _fetch_titles_by_keys(cfg: dict[str, Any], keys: set[tuple[int, int]]) -> list[dict[str, Any]]:
    """Batch-fetch CONTASPAGAR rows by (ID_DB, ID_CONTASPAGAR) — the true
    unique key (ID_CONTASPAGAR is numbered per ID_DB)."""
    if not keys:
        return []
    ...
    try:
        ...
        for i in range(0, len(klist), 400):
            chunk = klist[i:i + 400]
            ors = " OR ".join(
                f"(c.ID_DB={int(db)} AND c.ID_CONTASPAGAR={int(idc)})" for db, idc in chunk
            )
            cur.execute(f"SELECT c.* FROM dbo.CONTASPAGAR c WHERE {ors}")
...
def _key_db_idc(row: dict[str, Any]) -> tuple[int, int]:
    ...
    return (int(db), int(row["ID_CONTASPAGAR"]))
...
def close_phantoms(pg, cfg: dict[str, Any], cr_rows: list[dict[str, Any]], id_empresa: int, dry_run: bool) -> tuple[int, int]:
    ...
    STG-open title whose (ID_DB, ID_CONTASPAGAR) is absent from it is stale. We
    re-fetch those exact titles from Xpert and:
      ...
      * tombstone the rest (deleted/renumbered) with a ``TORQMIND_RECONCILED_ABSENT``
        payload marker so the finance_titles mart excludes them (migration 094).
    ...
    cur.execute(
        """
        SELECT id_filial, id_db, id_contaspagar
        FROM stg.contaspagar
        WHERE id_empresa = %s
    ...
    if found:
        reupserted = upsert_stg(
            pg, found, "contaspagar", "id_contaspagar", id_empresa,
    ...
    if deleted:
        ...
        for (f, db, idc) in deleted:
            cur.execute(
                """
                UPDATE stg.contaspagar
                SET payload = jsonb_set(payload, '{TORQMIND_RECONCILED_ABSENT}', to_jsonb(%s::text), true),
                    received_at = %s
                WHERE id_empresa = %s AND id_filial = %s AND id_db = %s AND id_contaspagar = %s
                """,
...
def refresh_dw_and_mart(pg, id_empresa: int) -> None:
    ...
    # skip it. force_full_scan re-reads every stg.contaspagar row (upsert is
    ...
    # Belt-and-suspenders: directly sync dw.fact_financeiro <- stg.contaspagar
    # for any payable whose payload still diverges. id_titulo (ID_CONTASPAGAR)
    ...
    pg.commit()
    if synced:
        log.info("DW payload sync (race-proof): %s payables reconciled DW<-STG", synced)
    # ClickHouse marts (finance_titles / finance_overview) are republished by the
    # wrapper script after CDC catches STG — not here (App VM may lack CH write).


# --------------------------------------------------------------------------- #
def main() -> None:
    ap = argparse.ArgumentParser(description="Reconcile CONTASPAGAR Xpert->TorqMind (hot window).")
    ...
    log.info("=== Reconcile CONTASPAGAR (id_empresa=%s, paid_days=%s, dry_run=%s) ===",
             args.id_empresa, args.paid_days, args.dry_run)

    cr_rows = extract_contaspagar(mssql, args.paid_days)
    bx_rows = extract_baixas(mssql, args.paid_days)

    pg = psycopg2.connect(**pg_dsn)
    try:
        cr_changed = upsert_stg(
            pg, cr_rows, "contaspagar", "id_contaspagar", args.id_empresa,
            change_keys=("DTAPGTO", "VLRPAGO"), dt_evento_key="DTACONTA", dry_run=args.dry_run,
        )
        bx_changed = upsert_stg(
            pg, bx_rows, "contaspagarbaixa", "id_contaspagarbaixa", args.id_empresa,
        ...
        if args.dry_run:
            log.info("[dry-run] DONE. contaspagar=%d contaspagarbaixa=%d phantoms(reupsert=%d,tombstone=%d) would change.",
    ...
    log.info("=== DONE. contaspagar changed=%d, contaspagarbaixa changed=%d, phantoms reupsert=%d tombstone=%d ===",
             cr_changed, bx_changed, ph_up, ph_tomb)
